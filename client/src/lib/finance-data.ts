...
export type FamilySettings = { familyName: string; memberName: string; familyCode: string; members: FamilyMember[]; paymentSources: PaymentSource[]; customCategories: string[]; quickTemplates: QuickTransactionTemplate[]; salaryPlan: SalaryPlan };
...
export const createEmptyAppData = (): AppData => ({
  ...
  settings: {
    ...
    customCategories: [], quickTemplates: [],
...
export const normalizeAppData = (input: unknown): AppData => {
  ...
  const quickTemplates = Array.isArray(oldSettings.quickTemplates) ? oldSettings.quickTemplates.map((item, index) => ({ id: item.id || `quick-template-${index}`, label: String(item.label || item.category || "Șablon rapid").trim(), kind: item.kind === "income" ? "income" as const : "expense" as const, category: item.kind === "income" ? "Venit" : String(item.category || "Alimente"), amount: Math.max(0, parseRomanianAmount(item.amount)), memberId: members.some((member) => member.id === item.memberId) ? item.memberId : undefined, sourceId: sources.some((source) => source.id === item.sourceId) ? item.sourceId : undefined, updatedAt: item.updatedAt || undefined })).filter((item) => item.label).slice(0, 12) : [];
  return {
