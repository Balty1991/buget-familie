 ...
 */
export type MonthlyNeed = { id: string; label: string; category: string; cadence: "monthly" | "weekly"; min: number; max: number; /** Cât se rezervă din interval. Implicit maximul (prudent). */ reserve?: "max" | "avg" | "min"; /** Doar din venitul acestui membru; lipsă = din oricare. */ payerId?: string; /** „fixed” (rate, facturi) se acoperă înaintea celor „flex” (mâncare, taxi); „buffer” (neprevăzute) doar din ce rămâne liber. */ priority?: "fixed" | "flex" | "buffer"; allocationId?: string; /** Luna („2026-10”) în care propunerea de ajustare a fost pusă sau lăsată; nu se mai cere până luna viitoare. */ reviewedMonth?: string; archived?: boolean; updatedAt?: string };
