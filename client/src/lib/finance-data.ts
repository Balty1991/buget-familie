...
export const normalizeAppData = (input: unknown): AppData => {
  ...
  const members = realRows<FamilyMember>(oldSettings.members).length ? realRows<FamilyMember>(oldSettings.members).map((member, index) => ({ id: member.id || `member-${index}`, name: member.name || `Membru ${index + 1}`, color: member.color, kind: member.kind === "child" ? "child" as const : undefined })) : [{ id: "member-me", name: memberName, color: "#256B5B" }];
