...
const family = (carry: boolean) => {
  ...
  data.settings.salaryPlan = { ...data.settings.salaryPlan, periodStart: "2026-10-05", nextPayday: "2026-11-01", paydayFlexDays: 0, weekCarryOver: carry || undefined, allocations: [{ id: "m", label: "Mâncare", amount: 2400, category: "Alimente", weeklyPace: true, weeklyAmount: 600 }] };
  data.transactions = [
    { id: "a", title: "Lidl", amount: 500, kind: "expense", category: "Alimente", source: "", person: "", date: "2026-10-07", allocationId: "m" },
    { id: "b", title: "Kaufland", amount: 800, kind: "expense", category: "Alimente", source: "", person: "", date: "2026-10-13", allocationId: "m" },
  ];
  return data;
};
...
describe("reportul între săptămâni", () => {
  ...
  it("fără report, fiecare săptămână are suma ei", () => {
    vi.useFakeTimers(); vi.setSystemTime(new Date("2026-10-20T10:00:00"));
  ...
  it("săptămâna în curs nu dă nimic mai departe până nu se încheie", () => {
    vi.useFakeTimers(); vi.setSystemTime(new Date("2026-10-12T10:00:00"));
    const data = family(true);
    const weeks = allocationWeeksStatus(data, data.settings.salaryPlan.allocations[0]);
