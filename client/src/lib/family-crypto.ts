...
export async function encryptFamilyData(data: AppData, secret: string): Promise<EncryptedEnvelope> {
  ...
  const shareable = { ...data, pendingReview: [], receipts: data.receipts.map(({ imageData: _one, imageData2: _two, imageKeys: _keys, ...receipt }) => receipt), settings: { ...data.settings, quickTemplates: [], archivedQuickTemplates: [], savedJournalFilters: [], salaryCycleTemplates: [], exchangeRates: [], seenWeeklyPlanTranches: [], basketProducts: [] } };
