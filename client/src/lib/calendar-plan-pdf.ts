...
export const downloadCalendarPlanPdf = async (plan: CalendarBudget, familyName: string) => {
  ...
  metric("SUMA DE PERIOADA", money(report.total), margin); metric("DURATA", `${report.days} zile · ${new Intl.NumberFormat(getLocale(), { maximumFractionDigits: 1 }).format(report.exactWeeks)} sapt.`, margin + 58); metric("RITM SAPTAMANAL", money(report.weeklyAmount), margin + 116); y += 32;
