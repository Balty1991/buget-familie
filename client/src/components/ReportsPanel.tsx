...
export function ReportsPanel({ data, onGo }: { data: AppData; onGo?: (view: MainView) => void }) {
  ...
  return <div className="bf-analysis">
    <section className="bf-analysis-control"><div><p className="bf-kicker">{compare.mode === "cycle" ? t("CITEȘTE CICLUL") : t("CITEȘTE LUNA")}</p><h2>{compare.mode === "cycle" ? compare.title : titleFor(focusMonth)}</h2><p>{compare.mode === "cycle" ? t("Comparația urmează ciclul de salariu, nu luna calendaristică — evită alarma falsă înainte să intre venitul.") : t("Perspectivele și valorile se calculează din mișcările înregistrate, nu dintr-un extras bancar.")}</p></div><label>{t("Luna analizată")}<input type="month" value={focusMonth} onChange={(event) => { setFocusMonth(event.target.value); setWindowMode("calendar"); }} /></label></section>
    <div className="bf-analysis-scope bf-analysis-window" role="group" aria-label={t("Fereastra de comparație")}><button type="button" className={compare.mode === "calendar" ? "active" : ""} onClick={() => setWindowMode("calendar")}>{t("Lună calendar")}</button><button type="button" className={compare.mode === "cycle" ? "active" : ""} disabled={!cycleReady} title={cycleReady ? t("Compară pe ciclul de salariu") : t("Setează data venitului în Plan pentru a compara pe ciclu.")} onClick={() => setWindowMode("cycle")}>{t("Ciclu salariu")}</button></div>
    ...
    <section className="bf-analysis-month"><div className="bf-analysis-month-heading"><div><p className="bf-kicker">{t("REZULTATUL LUNII · {perspective}", { perspective: perspective.toUpperCase() })}</p><h2>{currentFlow < 0 ? t("Au ieșit mai mulți bani decât au intrat.") : t("Luna rămâne în echilibru.")}</h2></div><span className={currentFlow < 0 ? "negative" : ""}>{money(currentFlow)}</span></div><div className="bf-analysis-flow"><article><span className="income"><ArrowDownRight size={16} /></span><div><small>{t("Venituri")}</small><b>{money(current.income)}</b><em>{change(current.income - prior.income, prior.income)}</em></div></article><article><span className="expense"><ArrowUpRight size={16} /></span><div><small>{t("Cheltuieli")}</small><b>{money(current.expense)}</b><em>{change(current.expense - prior.expense, prior.expense)}</em></div></article><article><span className="balance"><WalletCards size={16} /></span><div><small>{t("Bilanț înregistrat")}</small><b>{money(balance.cashflow)}</b><em>{change(currentFlow - priorFlow, priorFlow)}</em></div></article></div></section>
    <section className="bf-analysis-year"><div className="bf-analysis-section-heading"><div><p className="bf-kicker">{t("ANUL CURENT · {year}", { year })}</p><h2>{t("Ritm lună cu lună")}</h2></div><span>{t("atinge o lună pentru analiză")}</span></div><div className="bf-analysis-chart" aria-label={t("Evoluția lunară pentru {year}", { year })}>{monthly.map((item, index) => <button key={item.label} className={item.isFocus ? "active" : ""} onClick={() => { setFocusMonth(`${year}-${String(index + 1).padStart(2, "0")}`); setWindowMode("calendar"); setSelectedCategory(""); }} aria-label={t("{label}: venituri {income}, cheltuieli {expense}", { label: item.label, income: money(item.income), expense: money(item.expense) })}><span><i className="income" style={{ height: `${Math.max(3, item.income / maxMonthly * 100)}%` }} /><i className="expense" style={{ height: `${Math.max(3, item.expense / maxMonthly * 100)}%` }} /></span><b>{item.label}</b></button>)}</div><div className="bf-analysis-legend"><span><i className="income" /> {t("Venituri")}</span><span><i className="expense" /> {t("Cheltuieli")}</span><strong>{t("Ritm anual: {amount}", { amount: money(monthly.reduce((sum, item) => sum + item.flow, 0)) })}</strong></div></section>
    <section className="bf-spend-compass"><div className="bf-analysis-section-heading"><div><p className="bf-kicker">{t("DISTRIBUȚIE INTERACTIVĂ")}</p><h2>{t("Unde au mers banii")}</h2></div><TrendingDown size={19} /></div>{categorySlices.length ? <><div className="bf-spend-compass-layout"><div className="bf-spend-donut" style={{ background: categoryGradient }} role="img" aria-label={t("Distribuția cheltuielilor: {list}", { list: categorySlices.map((slice) => `${slice.name} ${Math.round(slice.share)}%`).join(", ") })}><div><small>{t("CHELTUIELI")}</small><b>{money(categoryTotal)}</b><span>{titleFor(focusMonth)}</span></div></div><div className="bf-spend-category-pills" role="list" aria-label={t("Categorii de cheltuieli")}>{categorySlices.map((slice) => <button key={slice.name} role="listitem" className={activeCategory?.name === slice.name ? "active" : ""} onClick={() => setSelectedCategory(slice.name)}><i style={{ backgroundColor: slice.color }} /><span><b>{slice.name}</b><small>{Math.round(slice.share)}% · {money(slice.value)}</small></span></button>)}</div></div>{activeCategory && <article className="bf-spend-focus" style={{ borderLeftColor: activeCategory.color }}><span style={{ backgroundColor: activeCategory.color }} /><div><small>{t("CATEGORIA SELECTATĂ")}</small><b>{activeCategory.name}</b><p>{t("{amount} · {share}% din cheltuielile acestei luni.", { amount: money(activeCategory.value), share: Math.round(activeCategory.share) })}</p></div></article>}<div className="bf-analysis-category-rows">{categories.map(([name, value]) => <button key={name} className={activeCategory?.name === name ? "active" : ""} onClick={() => setSelectedCategory(name)}><span>{name}</span><b>{money(value)}</b><i><em style={{ width: `${value / maxCategory * 100}%`, background: categoryColors[name] || "#73847b" }} /></i></button>)}</div></> : <div className="bf-empty-soft"><EmptyMark /><p>{t("Nu există cheltuieli în această fereastră pentru perspectiva aleasă.")}</p></div>}</section>
