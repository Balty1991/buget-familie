...
export function ReportsPanel({ data, onGo }: { data: AppData; onGo?: (view: MainView) => void }) {
  ...
  const nextStep = alerts.some((entry) => entry.state === "over")
    ...
    : currentFlow < 0
      ...
      : categories[0]
        ? { title: `Urmărește ${categories[0][0]}`, detail: `${money(categories[0][1])} reprezintă categoria principală din luna aleasă. Deschide registrul pentru a verifica mișcările care au format suma.`, label: "Vezi Mișcările", view: "journal" as MainView }
        : { title: "Înregistrează prima mișcare", detail: "Analiza devine mai utilă după ce există date reale în registru.", label: "Deschide Registrul", view: "journal" as MainView };
