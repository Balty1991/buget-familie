...
export function ReportsPanel({ data, onGo }: { data: AppData; onGo?: (view: MainView) => void }) {
  ...
  return <div className="bf-analysis">
    ...
    <div className="bf-analysis-scope bf-analysis-window" role="group" aria-label={t("Fereastra de comparație")}><button type="button" className={compare.mode === "calendar" ? "active" : ""} onClick={() => setWindowMode("calendar")}>{t("Lună calendar")}</button><button type="button" className={`${compare.mode === "cycle" ? "active" : ""}${cycleReady ? "" : " is-unavailable"}`} aria-expanded={cycleReady ? undefined : cycleHint} title={cycleReady ? t("Compară pe ciclul de salariu") : undefined} onClick={() => { if (cycleReady) setWindowMode("cycle"); else setCycleHint((open) => !open); }}>{t("Ciclu salariu")}</button></div>
    {!cycleReady && cycleHint && <p className="bf-notice bf-analysis-cycle-hint" role="status">{t("Ca să compari pe ciclul de salariu, spune întâi când vine venitul.")}{onGo && <> <button type="button" className="bf-link-button" onClick={() => onGo("plan")}>{t("Setează în Plan")}</button></>}</p>}
