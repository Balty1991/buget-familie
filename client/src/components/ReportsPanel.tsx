...
export function ReportsPanel({ data, onGo }: { data: AppData; onGo?: (view: MainView) => void }) {
  ...
  const change = (value: number, base: number) => base === 0 ? (value === 0 ? t("fără mișcări comparabile") : t("prima lună cu date")) : `${value >= 0 ? "+" : ""}${money(value)} față de luna anterioară`;
  /**
