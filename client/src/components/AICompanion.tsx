...
export function AICompanion({ data, view, onAdd, onGo, onNaturalEntry, onFinancialUpdate, onRevert }: Props) {
  ...
  const [undo, setUndo] = useState<(GuidedRevert & { label: string }) | null>(null);
  ...
  const applyChoice = (choice: ChatChoice) => {
    ...
    if ((update.kind === "expense" || update.kind === "income") && onRevert) {
      setUndo({ kind: update.kind, title: update.title, amount: update.amount, date: day, label: spent });
    }
  };
