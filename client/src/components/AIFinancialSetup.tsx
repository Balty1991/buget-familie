...
export function AIFinancialSetup({ data, onChange, onClose, onGoPlan, onAdd }: Props) {
  ...
  const steps = useMemo(() => [
    { label: "tu", icon: Sparkles, title: "Începem cu tine.", detail: "Îți pun câteva întrebări simple ca să construim situația ta financiară fără să cauți prin meniuri." },
    { label: "venit", icon: CircleDollarSign, title: "Ce venit intră în fiecare lună?", detail: "Poate fi salariu, pensie, freelancing sau orice venit repetitiv. Dacă variază, trecem o medie prudentă." },
    { label: "datorii", icon: CreditCard, title: "Ai datorii sau rate?", detail: "Le trecem acum ca să nu le uiți când îți planifici banii. Poți lăsa gol dacă nu ai." },
    { label: "repartizare", icon: Target, title: "Unde vrei să ajungă banii?", detail: "Spune-mi cât vrei să rezervi pentru fiecare zonă. Nu trebuie să fie perfect; ajustăm împreună." },
    { label: "solduri", icon: HandCoins, title: "Cu ce sold pornești?", detail: "Introdu aproximativ cât ai acum pe card, cash sau bonuri. La final verificăm împreună ce am înțeles." },
  ], []);
  ...
  const apply = () => {
    ...
    const newIncome = monthlyIncome > 0 && !data.transactions.some((item) => item.kind === "income" && item.title === "Venit lunar") ? [{ id: newId("income"), title: "Venit lunar", amount: monthlyIncome, kind: "income" as const, category: "Venit", sourceId: paymentSources[0]?.id, source: paymentSources[0]?.name || "Card debit", memberId, person: memberName, date: isoToday(), note: "Adăugat în configurarea inițială", createdAt: now }] : [];
  ...
  return <div className="bf-modal-backdrop bf-onboarding-backdrop" role="presentation"><section ref={dialogRef} tabIndex={-1} className="ai-financial-setup" role="dialog" aria-modal="true" aria-labelledby="ai-setup-title"><button type="button" className="ai-setup-skip" onClick={skip}>Mai târziu <X size={14} /></button><header className="ai-setup-header"><div className="ai-setup-avatar"><Sparkles size={20} /></div><div><p className="bf-kicker">GHIDUL TĂU AI · CONFIGURARE</p><strong>Îți pun întrebările potrivite.</strong></div></header><div className="ai-setup-progress">{steps.map((item, index) => <span key={item.label} className={index <= step ? "active" : ""} />)}</div><div className="ai-setup-chat"><div className="ai-setup-bubble ai-setup-bubble-ai"><Icon size={17} /><div><p className="bf-kicker">PASUL {step + 1} DIN {steps.length}</p><h2 id="ai-setup-title">{currentStep.title}</h2><p>{currentStep.detail}</p></div></div>{step === 0 && <div className="ai-setup-fields"><label className="bf-field"><span>Cum te cheamă?</span><input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="ex. Andrei" /></label><label className="bf-field"><span>Numele familiei (opțional)</span><input value={family} onChange={(event) => setFamily(event.target.value)} placeholder="ex. Familia Popescu" /></label></div>}{step === 1 && <div className="ai-setup-fields"><label className="bf-field"><span>Venit lunar mediu (lei)</span><input autoFocus inputMode="decimal" value={income} onChange={(event) => setIncome(event.target.value)} placeholder="ex. 6.500" /></label><label className="bf-field"><span>Când intră următorul venit?</span><input type="date" value={payday} onChange={(event) => setPayday(event.target.value)} /></label></div>}{step === 2 && <div className="ai-setup-fields"><label className="bf-field"><span>Ce datorie vrei să urmărești?</span><input autoFocus value={debtName} onChange={(event) => setDebtName(event.target.value)} placeholder="ex. Credit auto" /></label><div className="ai-setup-two"><label className="bf-field"><span>Sold rămas</span><input inputMode="decimal" value={debtRemaining} onChange={(event) => setDebtRemaining(event.target.value)} placeholder="0" /></label><label className="bf-field"><span>Rată lunară</span><input inputMode="decimal" value={debtMonthly} onChange={(event) => setDebtMonthly(event.target.value)} placeholder="0" /></label></div><p className="ai-setup-hint"><ShieldCheck size={14} /> Dacă ai mai multe, le poți adăuga ulterior din Obligații.</p></div>}{step === 3 && <div className="ai-setup-allocations">{allocations.map((item, index) => <div className="ai-setup-allocation" key={item.category}><label><b>{item.category}</b><small>{item.weekly ? "săptămânal" : "perioadă"}</small></label><input inputMode="decimal" value={item.amount} onChange={(event) => setAllocations((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, amount: event.target.value } : entry))} placeholder="0 lei" /><button type="button" aria-label={`Elimină ${item.category}`} onClick={() => setAllocations((current) => current.filter((_, entryIndex) => entryIndex !== index))}><X size={14} /></button></div>)}<button type="button" className="ai-add-allocation" onClick={() => setAllocations((current) => [...current, { category: "Altceva", amount: "", weekly: false }])}><Plus size={14} /> Adaugă altă destinație</button></div>}{step === 4 && <div className="ai-setup-fields"><p className="ai-setup-summary">Introdu soldurile aproximative de acum. Nu este nevoie să fie la virgulă; le putem ajusta oricând.</p>{data.settings.paymentSources.map((source) => <label className="bf-field" key={source.id}><span>{source.name}</span><input inputMode="decimal" value={balances[source.id] || ""} onChange={(event) => setBalances((current) => ({ ...current, [source.id]: event.target.value }))} placeholder="0" /></label>)}</div>}</div><footer className="ai-setup-footer"><span>{step === 4 ? "La final îți deschid tabloul." : "Poți reveni și modifica orice răspuns."}</span><button type="button" className="bf-primary" onClick={next}>{step === steps.length - 1 ? "Salvează și începe" : "Continuă"} <ArrowRight size={17} /></button></footer>{step === steps.length - 1 && <div className="ai-setup-quick-actions"><button type="button" onClick={() => { apply(); onClose(); onGoPlan(); }}><Target size={15} /> Vezi planul</button><button type="button" onClick={() => { apply(); onClose(); onAdd(); }}><Plus size={15} /> Adaugă prima mișcare</button></div>}</section></div>;
}
