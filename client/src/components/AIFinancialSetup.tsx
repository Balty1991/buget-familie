...
export function AIFinancialSetup({ data, onChange, onClose, onGoPlan, onAdd }: Props) {
  ...
  const steps = useMemo(() => [
    { label: t("tu"), icon: Sparkles, title: t("Începem cu tine."), detail: t("Îți pun câteva întrebări simple ca să construim situația ta financiară fără să cauți prin meniuri.") },
    { label: t("venit"), icon: CircleDollarSign, title: t("Ce venit intră în fiecare lună?"), detail: t("Poate fi salariu, pensie, freelancing sau orice venit repetitiv. Dacă variază, trecem o medie prudentă.") },
    { label: t("datorii"), icon: CreditCard, title: t("Ai datorii sau rate?"), detail: t("Le trecem acum ca să nu le uiți când îți planifici banii. Poți lăsa gol dacă nu ai.") },
    { label: t("repartizare"), icon: Target, title: t("Unde vrei să ajungă banii?"), detail: t("Spune-mi cât vrei să rezervi pentru fiecare zonă. Nu trebuie să fie perfect; ajustăm împreună.") },
    { label: t("solduri"), icon: HandCoins, title: t("Cu ce sold pornești?"), detail: t("Introdu aproximativ cât ai acum pe card, cash sau bonuri. La final verificăm împreună ce am înțeles.") },
  ], []);
  ...
  const apply = () => {
    ...
    safeSetItem(window.localStorage, "buget-familie:setup-complete", "true"); markSetupCompletedAt(window.localStorage);
