  ...
  return (
    <div className="bf-modal-backdrop bf-onboarding-backdrop" role="presentation">
      <section ref={dialogRef} tabIndex={-1} className="bf-onboarding bf-setup" role="dialog" aria-modal="true" aria-labelledby="bf-setup-title">
        ...
        {step === 1 && (
          <div className="bf-setup-copy">
            <h2 id="bf-setup-title">{t("Ce bani ai")} <em>acum?</em></h2>
            <p>{t("Scrie aproximativ cât ai în cont, pe card sau cash. Este doar punctul de plecare și poate fi schimbat mai târziu.")}</p>
        ...
        {step === 3 && (
          <div className="bf-setup-copy">
            ...
            <ul className="bf-setup-privacy">
              ...
              <li>{t("Poți șterge totul local oricând din Resetare.")}</li>
            </ul>
          </div>
        )}
        ...
        {step < 3 ? (
          <div className="bf-onboarding-actions"><button className="bf-primary" onClick={next}>{t("Continuă")} <ChevronRight size={17} /></button></div>
        ) : (
          <div className="bf-onboarding-actions bf-onboarding-choices">
            ...
            <button onClick={() => { apply(); complete(); onAdd(); }}>{t("Prima mișcare")}</button>
          </div>
        )}
      </section>
    </div>
