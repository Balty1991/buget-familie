...
export function PlanStudio({ data, onChange }: { data: AppData; onChange: (data: AppData) => void }) {
  ...
  return <div className="bf-page bf-plan-workspace bf-salary-cycle-plan">
    ...
    <section className={`bf-plan-flow ${planFlowOpen ? "expanded" : "compact"}`} aria-label={t("Progresul planului în trei pași")}><button type="button" className="bf-plan-flow-toggle" aria-expanded={planFlowOpen} onClick={() => setPlanFlowOpen((value) => !value)}><span>{planFlowOpen ? "Ascunde ghidul" : "Arată ghidul complet"}</span><ChevronDown size={16} /></button>
      ...
      <ol>
        <li className={periodValid ? "complete" : "active"}><span>01</span><div><b>Cadrul</b><small>{t("Perioadă opțională")}</small></div></li>
    ...
    <section className="bf-cycle-setup" aria-labelledby="cycle-setup-title">
      ...
      <div id="bf-allocation-builder" className="bf-allocation-builder">
        ...
        <PlanField label="Nume plic" hint={t("Poți scrie «Taxi soție» sau lăsa automat.")}><input value={allocationLabel} onChange={(event) => setAllocationLabel(event.target.value)} placeholder={t("ex. Alimente · card soție")} /></PlanField>
        ...
        <PlanField label={t("Plătit din")}><select value={allocationSourceId} onChange={(event) => setAllocationSourceId(event.target.value)}>{currentSourceOptions.map((source) => <option key={source.id} value={source.id}>{source.name} · {personName(data, source.memberId)} · {money(sourceBalance(data, source.id))}</option>)}</select></PlanField>
        ...
        <label className="bf-plan-toggle"><input type="checkbox" checked={allocationWeeklyPace} onChange={(event) => setAllocationWeeklyPace(event.target.checked)} /><span><b>{t("Împarte pe săptămâni")}</b><small>Dezactivează pentru plicuri fără ritm fix — taxi, cheltuieli ocazionale: rămâne doar totalul, fără presiune pe săptămână.</small></span></label>
        <div className="bf-allocation-builder-actions"><button className="bf-primary" onClick={() => setAllocationPreviewOpen(true)}><Plus size={17} /> {editingAllocationId ? "Salvează plicul" : "Adaugă plicul"}</button>{editingAllocationId && <button onClick={resetAllocationBuilder}>{t("Renunță")}</button>}</div>
      </div>
      {allocationPreviewOpen && <div className="bf-allocation-preview" role="dialog" aria-modal="true" aria-labelledby="allocation-preview-title"><div><p className="bf-kicker">PREVIZUALIZARE</p><h3 id="allocation-preview-title">{t("Verifică înainte de aplicare")}</h3><p>Vei {editingAllocationId ? "actualiza" : "adăuga"} plicul <b>{allocationLabel.trim() || allocationCategory}</b> cu <strong>{money(parseRomanianAmount(allocationAmount))}</strong> {t("pentru perioada aleasă.")}</p><div><span>{t("Rămas acum")}<strong>{money(unrepartized)}</strong></span><span>{t("Rămas după")}<strong>{money(unrepartized - parseRomanianAmount(allocationAmount))}</strong></span></div><small>{t("Previzualizarea nu schimbă nimic până când nu confirmi.")}</small><footer><button onClick={() => setAllocationPreviewOpen(false)}>{t("Înapoi la editare")}</button><button className="bf-primary" onClick={() => { setAllocationPreviewOpen(false); saveAllocation(); }}><Check size={16} /> {t("Confirmă repartizarea")}</button></footer></div></div>}
      ...
      <div className="bf-allocation-list bf-envelope-desk" aria-live="polite">
        {envelopes.map(({ item, budget, remaining, usage, state, week, weeks }) => <article key={item.id} className={state}>
          ...
          <p className="bf-allocation-why"><b>{state === "over" ? "De ce cere atenție" : state === "watch" ? "De ce apare aici" : "Cum se citește"}:</b> {state === "over" ? `Ai depășit limita cu ${money(Math.abs(remaining))}. Redu suma planificată sau revizuiește cheltuielile înainte de următorul venit.` : state === "watch" ? `${Math.round(usage * 100)}% din plic este consumat; mai ai ${money(Math.max(0, remaining))} pentru perioada aleasă.` : week ? `Mai ai ${money(Math.max(0, remaining))} în plic, iar ritmul săptămânal este ${money(week.budget)}.` : `Ai planificat ${money(budget)} pentru această categorie, fără presiune pe o tranșă săptămânală.`}</p>
        ...
        {!envelopes.length && <div className="bf-allocation-empty"><EnvelopeEmptyArt size={88} /><b>{t("Așază primii lei într-un plic.")}</b><span>Alege o categorie de mai sus sau completează formularul. Totalul planului este suma plicurilor — fără o limită generală separată.</span></div>}
            </div>
