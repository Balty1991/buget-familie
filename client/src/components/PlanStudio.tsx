...
export function PlanStudio({ data, onChange }: { data: AppData; onChange: (data: AppData) => void }) {
  ...
  return <div className="bf-page bf-plan-workspace bf-salary-cycle-plan">
    ...
    <header className="bf-plan-studio-header">
      <div><p className="bf-kicker">{t("PLANUL FAMILIEI, PE CATEGORII")}</p><h1>{t("Fiecare leu")} <em>{t("are un loc.")}</em></h1><p>{t("Adaugă câte o categorie cu suma ei. Totalul e suma categoriilor — nu introduci nicio sumă generală separat.")}</p></div>
