...
export function PlanStudio({ data, onChange }: { data: AppData; onChange: (data: AppData) => void }) {
  ...
  return <div className="bf-page bf-plan-workspace bf-salary-cycle-plan">
    ...
    <section className="bf-plan-simulator" aria-labelledby="bf-plan-simulator-title"><div><p className="bf-kicker">SCENARIU FĂRĂ RISC</p><h2 id="bf-plan-simulator-title">Testează planul înainte să-l schimbi.</h2><p>Modifică sumele într-o copie temporară. Registrul și planul real rămân intacte până când confirmi.</p></div><button type="button" className="bf-secondary" onClick={openSimulation} disabled={!plan.allocations.length}><Sparkles size={16} /> Deschide simularea</button></section>
    {simulationOpen && <section className="bf-plan-simulation" role="dialog" aria-modal="true" aria-labelledby="bf-plan-simulation-title"><div className="bf-plan-simulation-heading"><div><p className="bf-kicker">SIMULARE LOCALĂ</p><h2 id="bf-plan-simulation-title">Cum ar arăta o altă repartizare?</h2><p>Aceste valori sunt temporare și nu sunt salvate automat.</p></div><button type="button" className="bf-link-button" onClick={() => setSimulationOpen(false)}>Închide</button></div><div className="bf-plan-simulation-list">{plan.allocations.map((item) => <label key={item.id}><span><b>{item.label}</b><small>{item.category || "Categorie"} · acum {money(item.amount)}</small></span><input value={simulationAmounts[item.id] ?? String(item.amount)} onChange={(event) => setSimulationAmounts((current) => ({ ...current, [item.id]: event.target.value }))} inputMode="decimal" aria-label={`Suma simulată pentru ${item.label}`} /></label>)}</div><div className={`bf-plan-simulation-result ${simulationRemainder < 0 ? "negative" : "positive"}`}><span><small>Total simulat</small><b>{money(simulationTotal)}</b></span><span><small>Rămâne după scadențe</small><b>{money(simulationRemainder)}</b></span><span><small>Interpretare</small><b>{simulationRemainder < 0 ? "Peste disponibil" : simulationRemainder === 0 ? "Echilibru" : "Bani nealocați"}</b></span></div><p className="bf-plan-simulation-note">{simulationRemainder < 0 ? "Scenariul depășește banii disponibili. Redu una dintre sume înainte de aplicare." : simulationRemainder > 0 ? `După repartizare rămân ${money(simulationRemainder)} nealocați, disponibili pentru o nevoie viitoare.` : "Scenariul acoperă disponibilul și scadențele fără surplus."}</p><footer><button type="button" onClick={() => setSimulationOpen(false)}>Renunță</button><button type="button" className="bf-primary" disabled={simulationRemainder < 0} onClick={applySimulation}><Check size={16} /> Aplică scenariul</button></footer></section>}
    ...
    <section className="bf-cycle-setup" aria-labelledby="cycle-setup-title">
      ...
      <section className={`bf-allocation-guidance ${allocationHealth}`} aria-labelledby="bf-allocation-guidance-title"><div className="bf-allocation-guidance-heading"><div><p className="bf-kicker">REPARTIZARE GHIDATĂ</p><h2 id="bf-allocation-guidance-title">Înainte să adaugi un plic, vezi <em>ce mai trebuie acoperit.</em></h2><p>{allocationHealthLabel}. Plicurile sunt limite de planificare; nu mută bani din card sau cash.</p></div><WalletCards size={23} aria-hidden="true" /></div><div className="bf-allocation-guidance-stats"><span><small>Disponibil în surse</small><b>{money(Math.max(0, availableSources))}</b></span><span><small>În plicuri</small><b>{money(Math.max(0, reservedInEnvelopes))}</b></span><span><small>Scadențe</small><b>{money(Math.max(0, scheduled))}</b></span><span><small>De repartizat</small><b>{money(Math.max(0, unrepartized))}</b></span></div>{unrepartized > 0 && <button type="button" className="bf-allocation-guidance-action" onClick={() => { setAllocationAmount(String(Math.round(unrepartized))); setAllocationError(""); document.getElementById("bf-allocation-builder")?.scrollIntoView({ behavior: "smooth", block: "start" }); }}>Folosește suma nealocată pentru următorul plic <ChevronDown size={15} /></button>}{unrepartized < 0 && <p className="bf-form-error" role="alert">Limitele plicurilor și scadențele depășesc soldul disponibil. Redu un plic sau verifică sursele înainte de a continua.</p>}</section><p className="bf-allocation-intro">Adaugă o categorie pentru fiecare parte a banilor: alimente, taxi, abonamente, consumabile copil. La o cheltuială reală, alegi categoria și aplicația scade automat din plicul potrivit.</p>
