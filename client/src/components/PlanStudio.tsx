...
export function PlanStudio({ data, onChange }: { data: AppData; onChange: (data: AppData) => void }) {
  ...
  const exportCyclePdf = async () => { if (!activeCycle) return; try { await downloadCalendarPlanPdf(activeCycle, data.settings.familyName); } catch { setCycleError(t("PDF-ul nu a putut fi generat local. Încearcă din nou.")); } };
  ...
  return <div className="bf-page bf-plan-workspace bf-salary-cycle-plan">
    ...
    <section className={`bf-plan-flow ${planFlowOpen ? "expanded" : "compact"}`} aria-label={t("Progresul planului în trei pași")}><button type="button" className="bf-plan-flow-toggle" aria-expanded={planFlowOpen} onClick={() => setPlanFlowOpen((value) => !value)}><span>{planFlowOpen ? "Ascunde ghidul" : t("Arată ghidul complet")}</span><ChevronDown size={16} /></button>
      ...
      <ol>
        ...
        <li className={envelopes.length ? "complete" : "active"}><span>02</span><div><b>Plicurile</b><small>{envelopes.length ? `${envelopes.length} configurate` : t("Adaugă categorii")}</small></div></li>
        <li className={activeCycle ? "complete" : "upcoming"}><span>03</span><div><b>Ritmul</b><small>{activeCycle ? t("Gata de urmărit") : t("Se activează cu perioada")}</small></div></li>
      </ol>
    </section>
    ...
    <section className="bf-cycle-setup" aria-labelledby="cycle-setup-title">
      ...
      {allocationPreviewOpen && createPortal(<div className="bf-allocation-preview" role="dialog" aria-modal="true" aria-labelledby="allocation-preview-title"><div><p className="bf-kicker">PREVIZUALIZARE</p><h3 id="allocation-preview-title">{t("Verifică înainte de aplicare")}</h3><p>Vei {editingAllocationId ? "actualiza" : t("adăuga")} plicul <b>{allocationLabel.trim() || allocationCategory}</b> cu <strong>{money(parseRomanianAmount(allocationAmount))}</strong> {t("pentru perioada aleasă.")}</p><div><span>{t("Rămas acum")}<strong>{money(unrepartized)}</strong></span><span>{t("Rămas după")}<strong>{money(unrepartized - parseRomanianAmount(allocationAmount))}</strong></span></div><small>{t("Previzualizarea nu schimbă nimic până când nu confirmi.")}</small><footer><button onClick={() => setAllocationPreviewOpen(false)}>{t("Înapoi la editare")}</button><button className="bf-primary" onClick={() => { setAllocationPreviewOpen(false); saveAllocation(); }}><Check size={16} /> {t("Confirmă repartizarea")}</button></footer></div></div>, document.body)}
