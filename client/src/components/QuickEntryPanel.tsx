...
export function QuickEntryPanel({ data, onSave, onClose, onMore, onSaveTemplate, onDeleteTemplate, onArchiveTemplate, onRestoreTemplate, onDeleteArchivedTemplate }: Props) {
  ...
  const save = () => {
    ...
    if (foreign && !stored) return setError(`Adaugă în Setări cursul pentru ${foreign} înainte de a folosi această sursă.`);
  ...
  return <div className="bf-modal-backdrop" role="presentation" onPointerDown={closeIfBackdrop}>
    <section ref={dialogRef} tabIndex={-1} className="bf-modal bf-quick-entry-panel" role="dialog" aria-modal="true" aria-label={t("Înregistrare rapidă")} onPointerDown={(event) => event.stopPropagation()}>
      ...
      <div className="bf-quick-entry-grid"><label className="bf-field bf-amount-field"><span>Sumă ({isForeign ? entryCurrency : "lei"})</span><input autoFocus value={amount} onChange={(event) => setAmount(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); save(); } }} inputMode="decimal" placeholder="0,00" aria-describedby="bf-quick-amount-hint" />{recentAmounts.length > 0 && <span className="bf-amount-suggestions" id="bf-quick-amount-hint"><span>Folosit recent</span>{recentAmounts.map((value) => <button type="button" key={value} onClick={() => { setAmount(String(value)); setError(""); }}>{money.format(value)}</button>)}</span>}</label>{kind === "expense" ? <label className="bf-field"><span>Categorie</span><select value={category} onChange={(event) => { setCategory(event.target.value); setAllocationChoiceTouched(false); }}>{[...expenseCategories, ...data.settings.customCategories].map((item) => <option key={item}>{item}</option>)}</select></label> : <label className="bf-field"><span>{t("Ce venit?")}</span><input value={incomeLabel} onChange={(event) => setIncomeLabel(event.target.value)} placeholder="ex. Salariu, Bonus" /></label>}<label className="bf-field"><span>{t("Cine a înregistrat")}</span><select value={memberId} onChange={(event) => { setMemberId(event.target.value); setAllocationChoiceTouched(false); }}>{data.settings.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><label className="bf-field"><span>{kind === "expense" ? "Plătit din" : "Încasat în"}</span><select value={sourceId} onChange={(event) => { setSourceId(event.target.value); setAllocationChoiceTouched(false); }}>{data.settings.paymentSources.map((source) => <option key={source.id} value={source.id}>{source.name} · {sourceOwner(source.id)} · {money.format(sourceBalance(data, source.id))}{source.currency ? ` (${source.currency})` : ""}</option>)}</select></label></div>{isForeign && <section className={`bf-currency-preview ${convertedPreview ? "" : "pending"}`}><p className="bf-kicker">{t("SE ÎNREGISTREAZĂ ÎN LEI")}</p>{convertedPreview ? <><b>{money.format(convertedPreview)}</b><span>La cursul de {entryRate?.toLocaleString("ro-RO", { maximumFractionDigits: 4 })} lei pentru 1 {entryCurrency}, salvat în Setări.</span></> : <span>{entryRate ? `Completează suma în ${entryCurrency}.` : `Adaugă în Setări cursul pentru ${entryCurrency} înainte de a folosi această sursă.`}</span>}</section>}
      {parsedAmount > 0 && <p className={`bf-quick-source-preview ${projectedSourceBalance < 0 ? "over" : ""}`} aria-live="polite">{kind === "expense" ? "După această plată" : "După această încasare"}: <b>{money.format(Math.max(0, projectedSourceBalance))}</b> rămân în {data.settings.paymentSources.find((source) => source.id === sourceId)?.name || "sursa aleasă"}{projectedSourceBalance < 0 ? " — suma depășește soldul curent" : ""}.</p>}
      {kind === "expense" && <section className="bf-quick-envelope"><p className="bf-kicker">{t("PLICUL SĂPTĂMÂNII")}</p><label className="bf-field"><span>{t("Se consumă din")}</span><select value={allocationId} onChange={(event) => { setAllocationId(event.target.value); setAllocationChoiceTouched(true); }}><option value="outside">{t("În afara plicurilor — doar soldul sursei")}</option>{candidates.map((allocation) => { const activeWeek = allocation.weeklyPace === false ? undefined : allocationWeekStatus(data, allocation); const totalRemaining = allocation.weeklyPace === false ? allocationStatus(data, allocation).remaining : undefined; return <option key={allocation.id} value={allocation.id}>{allocation.label} · {activeWeek ? `${money.format(Math.max(0, activeWeek.remaining))} în S${activeWeek.index}` : totalRemaining !== undefined ? `${money.format(Math.max(0, totalRemaining))} rămași` : "fără tranșă activă"}</option>; })}</select></label>{matchedAllocation && <p className={(week && week.remaining - parseRomanianAmount(amount) < 0) || (matchedTotal && matchedTotal.remaining - parseRomanianAmount(amount) < 0) ? "over" : ""}>{week ? `S${week.index}: ${money.format(Math.max(0, week.remaining))} rămași din ${money.format(week.budget)}${parseRomanianAmount(amount) > 0 ? ` · după plată ${money.format(Math.max(0, week.remaining - parseRomanianAmount(amount)))}` : ""}` : matchedTotal ? `${money.format(Math.max(0, matchedTotal.remaining))} rămași din ${money.format(matchedTotal.budget)}${parseRomanianAmount(amount) > 0 ? ` · după plată ${money.format(Math.max(0, matchedTotal.remaining - parseRomanianAmount(amount)))}` : ""}` : "Plic selectat; încă nu este activă o tranșă calendaristică."}</p>}{!candidates.length && <p>{t("Nu există plic pentru această categorie și sursă. Poți salva în afara plicurilor.")}</p>}</section>}
