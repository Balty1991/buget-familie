/**
 * Atelierul Financiar — captură rapidă locală: șabloanele se salvează, se arhivează sau se restaurează fără a crea mișcări până la confirmare.
 */
import { useEffect, useMemo, useState } from "react";
import { Archive, ArchiveRestore, BookmarkPlus, Check, Plus, Trash2, X } from "lucide-react";
import { expenseCategories, formatDate, isoToday, newId, parseRomanianAmount, sourceBalance, type AppData, type QuickTransactionTemplate, type Transaction, type TransactionKind } from "@/lib/finance-data";
...
type Props = {
  ...
  onClose: () => void;
  onMore: () => void;
  onSaveTemplate: (item: QuickTransactionTemplate) => void;
  onDeleteTemplate: (id: string) => void;
  onArchiveTemplate: (id: string) => void;
  onRestoreTemplate: (id: string) => void;
  onDeleteArchivedTemplate: (id: string) => void;
};
...
export function QuickEntryPanel({ data, onSave, onClose, onMore, onSaveTemplate, onDeleteTemplate, onArchiveTemplate, onRestoreTemplate, onDeleteArchivedTemplate }: Props) {
  ...
  const selectTemplate = (template: QuickTransactionTemplate) => {
    ...
    if (template.memberId) setMemberId(template.memberId); if (template.sourceId) setSourceId(template.sourceId); setTemplateLabel(template.label); setError("");
  };
  const save = () => {
  ...
  const archive = () => {
    ...
    if (!window.confirm(`Arhivezi șablonul local „${activeTemplate.label}”? Îl poți restaura ulterior; tranzacțiile rămân neschimbate.`)) return;
    onArchiveTemplate(templateId); chooseManual();
  };
  const remove = () => {
    if (!templateId) return;
    if (!window.confirm(`Ștergi definitiv șablonul local „${activeTemplate?.label || "acesta"}”? Tranzacțiile rămân neschimbate.`)) return;
    onDeleteTemplate(templateId); chooseManual();
  };
  ...
  return <div className="bf-modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="bf-modal bf-quick-entry-panel" role="dialog" aria-modal="true" aria-label="Înregistrare rapidă" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <p className="bf-quick-entry-intro">Salvezi o mișcare reală cu data de azi. Alege un șablon sau completează manual.</p>
      ...
      {showArchive && <section className="bf-template-archive" aria-label="Arhiva lunară a șabloanelor">
        <p className="bf-kicker">ARHIVĂ LOCALĂ</p>
        {archiveGroups.map(([month, items]) => <div key={month}><h3>{formatDate(`${month}-01`, { month: "long", year: "numeric" })}</h3>{items.map((item) => <article key={item.id}><div><b>{item.label}</b><small>{item.amount ? money.format(item.amount) : "sumă liberă"} · arhivat {formatDate(item.archivedAt)}</small></div><div className="bf-template-archive-actions"><button type="button" onClick={() => onRestoreTemplate(item.id)}><ArchiveRestore size={15} /> Restaurează</button><button type="button" className="danger" aria-label={`Șterge definitiv ${item.label}`} onClick={() => removeArchived(item.id, item.label)}><Trash2 size={15} /></button></div></article>)}</div>)}
        {!archiveGroups.length && <p className="bf-empty-inline">Nu ai șabloane arhivate. Arhivează un șablon activ pentru a-l păstra în istoricul local.</p>}
      </section>}
      ...
      {kind === "expense" && <p className="bf-quick-entry-note">Această salvare rapidă nu consumă un plic. Pentru plic, notiță sau altă dată, folosește formularul complet.</p>}
      ...
      <button className="bf-primary full" onClick={save}><Check size={17} /> Salvează acum</button><button className="bf-quick-entry-more" onClick={onMore}><Plus size={16} /> Adaugă plic, notiță sau altă dată</button>
    </section>
  </div>;
}
