...
export function QuickEntryPanel({ data, onSave, onClose, onMore, onSaveTemplate, onDeleteTemplate, onArchiveTemplate, onRestoreTemplate, onDeleteArchivedTemplate, initialTemplateId }: Props) {
  ...
  const weeks = matchedAllocation && matchedAllocation.weeklyPace !== false ? allocationWeeksStatus(data, matchedAllocation) : [];
  ...
  const hideUnallocated = kind === "expense" && candidates.length > 0 && unrepartized < Math.max(0.005, parseRomanianAmount(amount) || 0);
  ...
  useEffect(() => {
    ...
    if (hideUnallocated && allocationId === "outside" && candidates[0]) {
      setAllocationId(candidates[0].id);
      return;
    }
  ...
  const save = () => {
    ...
    onSave({ id: captureIdRef.current, title: activeTemplate?.label || (kind === "expense" ? (merchant.trim() || t("Cheltuială rapidă · {category}", { category: t(category) })) : incomeLabel.trim() || t("Venit rapid")), amount: stored || numeric, originalAmount: foreign ? numeric : undefined, originalCurrency: foreign, exchangeRate: foreign ? rate : undefined, kind, category: kind === "expense" ? category : "Venit", sourceId: source.id, source: source.name, memberId: member.id, person: member.name, date: isoToday(), allocationId: kind === "expense" ? allocationId : undefined, createdAt: new Date().toISOString() }, { fromWeekIndex: kind === "expense" && matchedAllocation && matchedAllocation.weeklyPace !== false ? fromWeekIndex : undefined });
    onClose();
  };
  const remember = () => {
