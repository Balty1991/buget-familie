...
export function QuickEntryPanel({ data, onSave, onClose, onMore, onSaveTemplate, onDeleteTemplate, onArchiveTemplate, onRestoreTemplate, onDeleteArchivedTemplate }: Props) {
  ...
  const week = matchedAllocation ? allocationWeekStatus(data, matchedAllocation) : undefined;
  ...
  const selectTemplate = (template: QuickTransactionTemplate) => {
    ...
    if (template.memberId) setMemberId(template.memberId); if (template.sourceId) setSourceId(template.sourceId); setTemplateLabel(template.label); setAllocationChoiceTouched(false); setError("");
  };
  const save = () => {
    ...
    onSave({ id: newId("tx"), title: activeTemplate?.label || (kind === "expense" ? `Cheltuială rapidă · ${category}` : "Venit rapid"), amount: numeric, kind, category: kind === "expense" ? category : "Venit", sourceId: source.id, source: source.name, memberId: member.id, person: member.name, date: isoToday(), allocationId: kind === "expense" ? allocationId : undefined, createdAt: new Date().toISOString() });
    onClose();
  };
  const remember = () => {
  ...
  return <div className="bf-modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section ref={dialogRef} tabIndex={-1} className="bf-modal bf-quick-entry-panel" role="dialog" aria-modal="true" aria-label="Înregistrare rapidă" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="bf-quick-entry-grid"><label className="bf-field"><span>Sumă (lei)</span><input autoFocus value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" placeholder="0,00" /></label>{kind === "expense" && <label className="bf-field"><span>Categorie</span><select value={category} onChange={(event) => { setCategory(event.target.value); setAllocationChoiceTouched(false); }}>{[...expenseCategories, ...data.settings.customCategories].map((item) => <option key={item}>{item}</option>)}</select></label>}<label className="bf-field"><span>Cine a înregistrat</span><select value={memberId} onChange={(event) => { setMemberId(event.target.value); setAllocationChoiceTouched(false); }}>{data.settings.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><label className="bf-field"><span>{kind === "expense" ? "Plătit din" : "Încasat în"}</span><select value={sourceId} onChange={(event) => { setSourceId(event.target.value); setAllocationChoiceTouched(false); }}>{data.settings.paymentSources.map((source) => <option key={source.id} value={source.id}>{source.name} · {sourceOwner(source.id)} · {money.format(sourceBalance(data, source.id))}</option>)}</select></label></div>
      {kind === "expense" && <section className="bf-quick-envelope"><p className="bf-kicker">PLICUL SĂPTĂMÂNII</p><label className="bf-field"><span>Se consumă din</span><select value={allocationId} onChange={(event) => { setAllocationId(event.target.value); setAllocationChoiceTouched(true); }}><option value="outside">În afara plicurilor — doar soldul sursei</option>{candidates.map((allocation) => { const activeWeek = allocationWeekStatus(data, allocation); return <option key={allocation.id} value={allocation.id}>{allocation.label} · {activeWeek ? `${money.format(Math.max(0, activeWeek.remaining))} în S${activeWeek.index}` : "fără tranșă activă"}</option>; })}</select></label>{matchedAllocation && <p className={week && week.remaining - parseRomanianAmount(amount) < 0 ? "over" : ""}>{week ? `S${week.index}: ${money.format(Math.max(0, week.remaining))} rămași din ${money.format(week.budget)}${parseRomanianAmount(amount) > 0 ? ` · după plată ${money.format(Math.max(0, week.remaining - parseRomanianAmount(amount)))}` : ""}` : "Plic selectat; încă nu este activă o tranșă calendaristică."}</p>}{!candidates.length && <p>Nu există plic pentru această categorie și sursă. Poți salva în afara plicurilor.</p>}</section>}
