...
/** Câte zile de registru se arată dintr-o dată; restul, la cerere. */
...
export function MovementsJournal({ data, onEdit, onDelete, onAdd, onOpenReview, onChange }: { data: AppData; onEdit: (item: Transaction) => void; onDelete: (id: string) => void; onAdd: () => void; onOpenReview?: () => void; onChange?: (next: AppData) => void }) {
  ...
  const deleteFilter = (filter: AppData["settings"]["savedJournalFilters"][number]) => { if (window.confirm(t("Ștergi filtrul local „{label}”?", { label: filter.label }))) saveLocalFilters(data.settings.savedJournalFilters.filter((item) => item.id !== filter.id)); };
