    ...
    <div className="bf-modal-backdrop bf-theme-backdrop" role="presentation" onPointerDown={closeIfBackdrop}>
      <section ref={dialogRef} tabIndex={-1} className="bf-modal bf-theme-picker" role="dialog" aria-modal="true" aria-label="Alege aspectul" onPointerDown={(event) => event.stopPropagation()}>
        ...
        <section className={`bf-theme-preview ${preview} background-preview-${previewBackground}`} aria-label={`Previzualizare ${previewOption.name}`}>
          ...
          <div className="bf-theme-preview-stats"><span>SURSE UTILIZABILE <b>4.830 RON</b></span><span>PUS DEOPARTE <b>780 RON</b></span></div>
...
export function CalmOnboarding({ onClose, onAdd, onGo }: { onClose: () => void; onAdd: () => void; onGo: (view: MainView) => void }) {
  ...
  const dialogRef = useFocusTrap<HTMLElement>(onClose);
  const steps = [
    { kicker: "CALM FINANCIAL", title: "Banii tăi, pe înțelesul tău.", emphasis: "Fără calcule complicate.", detail: "Începi cu ce ai acum, iar aplicația te ajută să vezi ce ai folosit și ce mai rămâne.", icon: WalletCards, tone: "calm" },
    { kicker: "01 · ÎMPĂRȚIM", title: "Punem banii în locul potrivit.", emphasis: "Pas cu pas.", detail: "Un plic este o sumă pusă deoparte pentru un scop: mâncare, facturi, transport sau orice contează pentru tine.", icon: Goal, tone: "envelope" },
    { kicker: "02 · AZI", title: "Vezi ce poți folosi azi.", emphasis: "Fără presupuneri.", detail: "Ecranul Astăzi îți arată cât a mai rămas în plicuri, ce plăți urmează și care este următorul pas simplu.", icon: CalendarClock, tone: "rhythm" },
    { kicker: "03 · PRIMUL PAS", title: "Începe cu o singură", emphasis: "cheltuială sau încasare.", detail: "Nu trebuie să completezi totul acum. Adaugă un singur lucru și construim de acolo.", icon: Plus, tone: "start" },
  ];
...
export function TransactionForm({ data, initial, onSave, onClose }: { data: AppData; initial?: Transaction; onSave: (item: Transaction) => void; onClose: () => void }) {
  ...
  /**
   * Sursa aleasă dă valuta. Utilizatorul tastează suma așa cum apare pe extras, iar
   * echivalentul în lei este calculat și arătat înainte de salvare — registrul rămâne
   * în lei, dar suma originală se păstrează pentru verificare.
   */
  ...
  const envelopeCandidates = kind === "expense" ? matchingAllocationsForExpense(data, { category, memberId, sourceId }) : []; const envelopeCandidateIds = envelopeCandidates.map((item) => item.id).join("|"); const matchedEnvelope = allocationId === "outside" ? undefined : envelopeCandidates.find((allocation) => allocation.id === allocationId); const sourceOwner = (source: AppData["settings"]["paymentSources"][number]) => data.settings.members.find((member) => member.id === source.memberId)?.name || "Comun"; const allocationMember = matchedEnvelope ? data.settings.members.find((member) => member.id === matchedEnvelope.memberId)?.name || "Familie / comun" : ""; const editedAlreadyInEnvelope = Boolean(matchedEnvelope && initial?.id && initial.allocationId === matchedEnvelope.id); const envelopeSpent = matchedEnvelope ? Math.max(0, allocationSpent(data, matchedEnvelope) - (editedAlreadyInEnvelope ? initial?.amount || 0 : 0)) : 0; const envelopeRemaining = matchedEnvelope ? allocationBudget(data, matchedEnvelope) - envelopeSpent : 0; const matchedWeek = matchedEnvelope && matchedEnvelope.weeklyPace !== false ? allocationWeekStatus(data, matchedEnvelope, date) : undefined; const initialInsideMatchedWeek = Boolean(initial && initial.date && matchedWeek && initial.date >= matchedWeek.start && initial.date <= matchedWeek.end); const adjustedWeekSpent = matchedWeek ? Math.max(0, matchedWeek.spent - (editedAlreadyInEnvelope && initialInsideMatchedWeek ? initial?.amount || 0 : 0)) : 0; const weekRemaining = matchedWeek ? matchedWeek.budget - adjustedWeekSpent : 0; const proposedAmount = baseAmount || 0; const envelopeAfter = envelopeRemaining - proposedAmount; const weekAfter = weekRemaining - proposedAmount;
  // La schimbarea sursei preluăm cursul salvat pentru noua valută, ca utilizatorul să nu-l retasteze.
  ...
  const save = () => {
    ...
    if (isForeign && !(activeRate > 0)) return setError(`Introdu cursul pentru ${entryCurrency}: câți lei face o unitate.`);
    const stored = isForeign ? toBaseAmount(numeric, activeRate) : numeric;
    if (!stored || stored <= 0) return setError("Suma convertită în lei nu este validă. Verifică suma și cursul.");
    ...
    onSave({ id: initial?.id || newId("tx"), title: title.trim(), amount: stored, originalAmount: isForeign ? numeric : undefined, originalCurrency: isForeign ? entryCurrency : undefined, exchangeRate: isForeign ? activeRate : undefined, kind, category: kind === "income" ? "Venit" : category, sourceId: source.id, source: source.name, memberId: member.id, person: member.name, date, note: note.trim() || undefined, allocationId: kind === "expense" ? allocationId : undefined, createdAt: initial?.createdAt || new Date().toISOString(), receiptId: initial?.receiptId });
    onClose();
  };
  return <Modal title={initial ? "Corectează mișcarea" : "Adaugă mișcare"} onClose={onClose}><div className="bf-segment"><button className={kind === "expense" ? "active expense" : ""} onClick={() => setKind("expense")}>{t("Cheltuială")}</button><button className={kind === "income" ? "active income" : ""} onClick={() => setKind("income")}>Venit</button></div><div className="bf-form-grid"><Field label="Denumire"><input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t("ex. Cumpărături Lidl")} /></Field><Field label={`Sumă (${isForeign ? entryCurrency : "lei"})`}><input value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" placeholder="0,00" /></Field>{isForeign && <Field label={`Curs: 1 ${entryCurrency} = ? lei`} hint={savedRate ? `Cursul salvat în Setări este ${savedRate.toLocaleString("ro-RO", { maximumFractionDigits: 4 })}. Îl poți schimba doar pentru această mișcare.` : "Nu ai încă un curs salvat pentru această valută. Îl poți pune o dată, în Setări."}><input value={rateInput} onChange={(event) => setRateInput(event.target.value)} inputMode="decimal" placeholder="ex. 4,97" /></Field>}<Field label="Data"><input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></Field><Field label={t("Cine a făcut mișcarea")}><select value={memberId} onChange={(event) => setMemberId(event.target.value)}>{data.settings.members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></Field><Field label={kind === "income" ? "Încasat în" : "Plătit din (sursa reală)"}><select value={sourceId} onChange={(event) => setSourceId(event.target.value)}>{data.settings.paymentSources.map((source) => <option key={source.id} value={source.id}>{source.name} · {sourceOwner(source)} · {money(sourceBalance(data, source.id))}{source.currency ? ` (${source.currency})` : ""}</option>)}</select></Field>{kind === "expense" && <Field label="Categorie"><select value={category} onChange={(event) => setCategory(event.target.value)}>{[...expenseCategories, ...data.settings.customCategories].map((item) => <option key={item}>{item}</option>)}</select></Field>}</div>{isForeign && <section className={`bf-currency-preview ${baseAmount ? "" : "pending"}`}><p className="bf-kicker">{t("SE ÎNREGISTREAZĂ ÎN LEI")}</p>{baseAmount ? <><b>{fmtExact.format(baseAmount)}</b><span>{fmtExact.format(typedAmount).replace("RON", entryCurrency)} × {activeRate.toLocaleString("ro-RO", { maximumFractionDigits: 4 })} lei. Suma originală și cursul rămân salvate lângă mișcare.</span></> : <span>{t("Completează suma și cursul ca să vezi echivalentul în lei.")}</span>}</section>}{kind === "expense" && <section className="bf-envelope-choice"><div><p className="bf-kicker">BUGET REPARTIZAT</p><h3>{t("Plicul compatibil este ales automat.")}</h3><p>{t("Categoria, membrul și sursa reală găsesc plicul potrivit. Poți alege alt plic sau plată în afara plicurilor.")}</p></div><Field label={t("Plic de consum")}><select value={allocationId} onChange={(event) => { setAllocationId(event.target.value); setAllocationChoiceTouched(true); }}><option value="outside">{t("În afara plicurilor — nu consumă buget repartizat")}</option>{envelopeCandidates.map((allocation) => { const owner = data.settings.members.find((member) => member.id === allocation.memberId)?.name || "Familie / comun"; const source = data.settings.paymentSources.find((item) => item.id === allocation.sourceId); const remaining = Math.max(0, allocationBudget(data, allocation) - allocationSpent(data, allocation)); const week = allocation.weeklyPace === false ? undefined : allocationWeekStatus(data, allocation, date); return <option key={allocation.id} value={allocation.id}>{allocation.label} · {owner} · {source?.name || "orice sursă"} · {week ? `${money(Math.max(0, week.remaining))} în S${week.index}` : money(remaining)}</option>; })}</select></Field>{!envelopeCandidates.length && <small className="bf-envelope-empty">Nu există un plic pentru această combinație de categorie, membru și sursă. Poți înregistra cheltuiala în afara plicurilor sau crea unul în Plan.</small>}</section>}{matchedEnvelope ? <section className={`bf-envelope-match ${envelopeAfter < 0 || (matchedWeek && weekAfter < 0) ? "over" : ""}`}><p>{matchedWeek ? "SE VA LUA DIN PLICUL SĂPTĂMÂNII ACTIVE" : "SE VA LUA DIN PLIC"}</p><b>{matchedEnvelope.label} · {allocationMember} · {data.settings.paymentSources.find((source) => source.id === matchedEnvelope.sourceId)?.name || "sursa aleasă"}</b>{matchedWeek ? <span>S{matchedWeek.index}: {money(Math.max(0, weekRemaining))} rămași din {money(matchedWeek.budget)}{proposedAmount > 0 ? weekAfter < 0 ? ` · depășești tranșa cu ${money(Math.abs(weekAfter))}` : ` · după plată rămân ${money(weekAfter)}` : ""}</span> : <span>{money(Math.max(0, envelopeRemaining))} rămași din limita ajustată de {money(allocationBudget(data, matchedEnvelope))}</span>}{matchedEnvelope.note && <small>{matchedEnvelope.note}</small>}</section> : kind === "expense" && <section className="bf-envelope-match outside"><p>{t("PLATĂ ÎN AFARA PLICURILOR")}</p><b>{t("Va scădea doar soldul sursei reale de plată.")}</b><span>{t("Nu consumă nicio limită repartizată pentru categorii.")}</span></section>}<Field label={t("Notiță opțională")}><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={t("ex. cursă taxi, traseu, persoană, motiv")} /></Field>{error && <p className="bf-form-error" role="alert">{error}</p>}<button className="bf-primary full" onClick={save}><Check size={17} /> {t("Salvează mișcarea")}</button></Modal>;
}
...
function DebtSchedule({ data, debt, onPay }: { data: AppData; debt: Debt; onPay: () => void }) {
  ...
  return <div className="bf-debt-schedule"><div className="bf-debt-schedule-head"><div><p>{t("SCADENȚAR COMPLET")}</p><b>{paidCount} din {rows.length} rate bifate</b></div><span>{money(debt.remaining)} rămas</span></div><div className="bf-debt-schedule-list">{visible.map((row) => <div className={"bf-debt-schedule-row " + (row.paid ? "paid" : "")} key={row.date}><button type="button" className="bf-schedule-check" aria-label={row.paid ? "Rata " + row.index + " achitată" : "Confirmă rata " + row.index} onClick={row.paid ? undefined : onPay}>{row.paid ? <Check size={15} /> : <span />}</button><span><b>Rata {String(row.index).padStart(2, "0")}</b><small>{dateText(row.date, true)}{row.paid ? " · plătită la " + dateText(row.paid.date, true) : " · neplătită"}</small></span><strong>{money(row.paid?.amount || row.amount)}</strong></div>)}</div>{rows.length > 6 && <button type="button" className="bf-schedule-more" onClick={() => setExpanded((value) => !value)}>{expanded ? "Arată mai puține" : "Arată toate cele " + rows.length + " rate"}</button>}</div>;
}
function DebtPayoffPlan({ data }: { data: AppData }) {
  ...
  const finishText = finish ? finish.toLocaleDateString(getLocale(), { month: "long", year: "numeric" }) : "adaugă rate lunare";
  return <section className="bf-debt-plan"><div className="bf-debt-plan-top"><div><p className="bf-kicker">{t("PLANUL DE IEȘIRE")}</p><h2>{t("Mai sunt aproximativ")} <em>{months ?? "—"} luni</em>.</h2><span>{t("La ritmul minim actual, datoriile pot fi închise până în")} <b>{finishText}</b>.</span></div><div className="bf-debt-plan-total"><strong>{money(total)}</strong><small>sold total</small></div></div><div className="bf-debt-plan-progress"><div><span>Progres real</span><b>{progress}%</b></div><i><em style={{ width: progress + "%" }} /></i></div><div className="bf-debt-plan-stats"><span><b>{money(monthly)}</b><small>{t("rate / lună")}</small></span><span><b>{openDebts.length}</b><small>datorii active</small></span><span><b>{money(paidTotal)}</b><small>{t("achitat până acum")}</small></span></div></section>;
}
...
export function SettingsPanel({ data, onChange, onReset }: { data: AppData; onChange: (value: AppData) => void; onReset: () => void }) {
  ...
  return <div className="bf-settings"><LanguageSection /><section><p className="bf-kicker">FAMILIE</p><Field label={t("Numele familiei")}><input value={settings.familyName} onChange={(event) => change({ familyName: event.target.value })} /></Field><Field label={t("Numele tău")}><input value={settings.memberName} onChange={(event) => change({ memberName: event.target.value })} /></Field><Field label={t("Cod local de familie")} hint="Nu leagă telefoanele. E doar un semn în backup-ul exportat, ca să recunoști fișierul. Conectarea se face din Sync, cu parola de 12+ caractere."><input value={settings.familyCode} onChange={(event) => change({ familyCode: event.target.value.toUpperCase() })} /><button className="bf-link-button" onClick={() => change({ familyCode: createFamilyCode() })}><RotateCcw size={14} /> Cod nou</button></Field></section><section><p className="bf-kicker">MEMBRI</p><div className="bf-member-rows">{settings.members.map((item) => <div className="bf-member-row" key={item.id}><b>{item.name}</b><label><input type="checkbox" checked={item.kind === "child"} onChange={(event) => change({ members: settings.members.map((memberItem) => memberItem.id === item.id ? { ...memberItem, kind: event.target.checked ? "child" as const : undefined } : memberItem) })} /> Copil</label>{settings.members.length > 1 && <button aria-label={`Șterge membrul ${item.name}`} onClick={() => change({ members: settings.members.filter((memberItem) => memberItem.id !== item.id) })}><X size={14} /></button>}</div>)}</div><small className="bf-helper">{t("Un membru marcat drept copil primește un ecran simplu de buzunar. Dă-i un plic pe numele lui, din Plan.")}</small><div className="bf-mini-form"><input value={member} onChange={(event) => setMember(event.target.value)} placeholder={t("ex. Soția")} /><button onClick={addMember}>{t("Adaugă")}</button></div></section><section><p className="bf-kicker">{t("SURSE ȘI SOLD INITIAL")}</p>{settings.paymentSources.map((item) => { const own = sourceBalanceInCurrency(data, item.id); return <div className="bf-source-edit" key={item.id}><span><b>{item.name}</b><small>{sourceKindName[item.kind]} · {settings.members.find((memberItem) => memberItem.id === item.memberId)?.name || "Comun"}{own ? ` · ${own.amount.toLocaleString("ro-RO", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${own.currency}${own.exact ? "" : " (aproximativ)"}` : ""}</small></span><label><small>Sold inițial ({item.currency || "lei"})</small><input inputMode="decimal" value={String(item.openingBalance)} onChange={(event) => change({ paymentSources: settings.paymentSources.map((sourceItem) => sourceItem.id === item.id ? { ...sourceItem, openingBalance: Math.max(0, parseRomanianAmount(event.target.value)) } : sourceItem) })} /></label><label><small>{t("Valută")}</small><select value={item.currency || BASE_CURRENCY} onChange={(event) => change({ paymentSources: settings.paymentSources.map((sourceItem) => sourceItem.id === item.id ? { ...sourceItem, currency: event.target.value === BASE_CURRENCY ? undefined : event.target.value } : sourceItem) })}>{supportedCurrencies.map((code) => <option key={code} value={code}>{code}</option>)}</select></label><strong>{money(sourceBalance(data, item.id))}</strong></div>; })}<div className="bf-source-builder"><input value={source} onChange={(event) => setSource(event.target.value)} placeholder={t("ex. Card soție")} /><select value={kind} onChange={(event) => setKind(event.target.value as PaymentKind)}>{Object.entries(sourceKindName).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select><select value={owner} disabled={kind === "transfer"} onChange={(event) => setOwner(event.target.value)}>{settings.members.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={currency} onChange={(event) => setCurrency(event.target.value)} aria-label="Valuta sursei">{supportedCurrencies.map((code) => <option key={code} value={code}>{code}</option>)}</select><button onClick={addSource}>{t("Adaugă")}</button></div></section><ExchangeRatesSection data={data} settings={settings} change={change} /><section><p className="bf-kicker">CATEGORII PROPRII</p><div className="bf-chip-list">{settings.customCategories.map((item) => <span key={item}>{item}<button onClick={() => change({ customCategories: settings.customCategories.filter((categoryItem) => categoryItem !== item) })}><X size={14} /></button></span>)}</div><div className="bf-mini-form"><input value={category} onChange={(event) => setCategory(event.target.value)} placeholder="ex. Taxi" /><button onClick={() => { if (category.trim() && !settings.customCategories.includes(category.trim())) { change({ customCategories: [...settings.customCategories, category.trim()] }); setCategory(""); } }}>{t("Adaugă")}</button></div></section><section className="bf-settings-notifications"><p className="bf-kicker">ALERTE LOCALE</p><h2>{t("Reamintiri pe telefon")}</h2><p>{t("Scadențe, plicuri aproape de limită și un check-in calm. Rămân pe dispozitiv; nu se trimit pe server.")}</p><div className="bf-notification-actions"><button type="button" className="bf-primary" onClick={() => void (async () => { const status = await requestNotificationPermission(); setNotificationsEnabled(status === "granted"); if (status === "granted") window.alert("Alertele sunt active pe acest dispozitiv."); else if (status === "denied") window.alert("Permisiunea a fost refuzată. O poți reactiva din setările telefonului."); else window.alert("Notificările nu sunt disponibile în acest browser."); })()}>{t("Activează alertele")}</button><button type="button" onClick={() => { setNotificationsEnabled(false); window.alert("Alertele au fost oprite pe acest dispozitiv."); }}>{t("Oprește alertele")}</button></div><small className="bf-helper">{t("Pe Android nativ:")} <code>pnpm install && npx cap sync</code>.</small></section><AppLockSettings /><section><p className="bf-kicker">{t("BACKUP ȘI RECUPERARE")}</p><h2>{t("Protejează registrul local")}</h2><p>{t("Backup-ul este un fișier local JSON. Nu este trimis automat în rețea și poate fi importat pe un alt dispozitiv.")}</p><div className="bf-backup-actions"><button onClick={() => downloadBackup(data)}><Download size={16} /> {t("Exportă backup")}</button><label className="bf-file-button"><Upload size={16} /> Alege backup<input type="file" accept="application/json,.json" onChange={importBackup} /></label></div>{backupPreview && <div className="bf-backup-preview" role="alert" aria-live="polite"><div><p className="bf-kicker">{t("PREVIZUALIZARE ÎNAINTE DE IMPORT")}</p><h3>{backupPreview.fileName}</h3><p>Exportat la {new Intl.DateTimeFormat(getLocale(), { dateStyle: "medium", timeStyle: "short" }).format(new Date(backupPreview.exportedAt))}.</p><div className="bf-backup-preview-stats"><span><b>{backupPreview.data.transactions.length}</b><small>{t("mișcări")}</small></span><span><b>{backupPreview.data.receipts.length}</b><small>bonuri</small></span><span><b>{backupPreview.data.settings.members.length}</b><small>membri</small></span></div></div><p className="bf-backup-warning">{t("Importul va înlocui datele locale actuale. Nimic nu se schimbă până nu confirmi.")}</p><div className="bf-backup-preview-actions"><button type="button" onClick={() => setBackupPreview(null)}>{t("Anulează")}</button><button type="button" className="bf-primary" onClick={confirmBackupImport}><Check size={16} /> {t("Confirmă importul")}</button></div></div>}</section><Suspense fallback={null}><TrustCenter /></Suspense><Suspense fallback={null}><PremiumStudio /></Suspense><section className="bf-danger"><p className="bf-kicker">RESETARE</p><h2>{t("Începe curat pe acest dispozitiv")}</h2><p>{t("Șterge numai datele locale. O copie sincronizată sau exportată nu este afectată.")}</p><button onClick={onReset}><Trash2 size={16} /> {t("Resetează datele locale")}</button></section></div>;
}
