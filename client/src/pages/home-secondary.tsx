...
export function CalmOnboarding({ onClose, onAdd, onGo }: { onClose: () => void; onAdd: () => void; onGo: (view: MainView) => void }) {
  ...
  const dialogRef = useFocusTrap<HTMLElement>(onClose);
  const steps = [
    { kicker: "CALM FINANCIAL", title: t("Banii tăi, pe înțelesul tău."), emphasis: t("Fără calcule complicate."), detail: t("Începi cu ce ai acum, iar aplicația te ajută să vezi ce ai folosit și ce mai rămâne."), icon: WalletCards, tone: "calm" },
