    ...
    <div className="bf-modal-backdrop bf-theme-backdrop" role="presentation" onPointerDown={closeIfBackdrop}>
      <section ref={dialogRef} tabIndex={-1} className="bf-modal bf-theme-picker" role="dialog" aria-modal="true" aria-label="Alege aspectul" onPointerDown={(event) => event.stopPropagation()}>
        ...
        <section className={`bf-theme-preview ${preview} background-preview-${previewBackground}`} aria-label={`Previzualizare ${previewOption.name}`}>
          ...
          <div className="bf-theme-preview-value"><small>RĂMAS ÎN PLICURI</small><strong>1.480 RON</strong><i /></div>
...
function DebtSchedule({ data, debt, onPay }: { data: AppData; debt: Debt; onPay: () => void }) {
  ...
  return <div className="bf-debt-schedule"><div className="bf-debt-schedule-head"><div><p>SCADENȚAR COMPLET</p><b>{paidCount} din {rows.length} rate bifate</b></div><span>{money(debt.remaining)} rămas</span></div><div className="bf-debt-schedule-list">{visible.map((row) => <div className={"bf-debt-schedule-row " + (row.paid ? "paid" : "")} key={row.date}><button type="button" className="bf-schedule-check" aria-label={row.paid ? "Rata " + row.index + " achitată" : "Confirmă rata " + row.index} onClick={row.paid ? undefined : onPay}>{row.paid ? <Check size={15} /> : <span />}</button><span><b>Rata {String(row.index).padStart(2, "0")}</b><small>{dateText(row.date, true)}{row.paid ? " · plătită la " + dateText(row.paid.date, true) : " · neplătită"}</small></span><strong>{money(row.paid?.amount || row.amount)}</strong></div>)}</div>{rows.length > 6 && <button type="button" className="bf-schedule-more" onClick={() => setExpanded((value) => !value)}>{expanded ? "Arată mai puține" : "Arată toate cele " + rows.length + " rate"}</button>}</div>;
}
function DebtPayoffPlan({ data }: { data: AppData }) {
  const openDebts = data.debts.filter((debt) => debt.remaining > 0);
  if (!openDebts.length) return <section className="bf-debt-plan complete"><div className="bf-debt-plan-mark"><Check size={22} /></div><div><p className="bf-kicker">PLANUL DE IEȘIRE</p><h2>Nu mai ai datorii active.</h2><span>Ai închis toate obligațiile înregistrate. Următorul pas poate fi un fond de siguranță.</span></div></section>;
  ...
  const finishText = finish ? finish.toLocaleDateString("ro-RO", { month: "long", year: "numeric" }) : "adaugă rate lunare";
  return <section className="bf-debt-plan"><div className="bf-debt-plan-top"><div><p className="bf-kicker">PLANUL DE IEȘIRE</p><h2>Mai sunt aproximativ <em>{months ?? "—"} luni</em>.</h2><span>La ritmul minim actual, datoriile pot fi închise până în <b>{finishText}</b>.</span></div><div className="bf-debt-plan-total"><strong>{money(total)}</strong><small>sold total</small></div></div><div className="bf-debt-plan-progress"><div><span>Progres real</span><b>{progress}%</b></div><i><em style={{ width: progress + "%" }} /></i></div><div className="bf-debt-plan-stats"><span><b>{money(monthly)}</b><small>rate / lună</small></span><span><b>{openDebts.length}</b><small>datorii active</small></span><span><b>{money(paidTotal)}</b><small>achitat până acum</small></span></div></section>;
}
function DebtPayoffSimulator({ data }: { data: AppData }) {
  ...
  return <section className="bf-debt-simulator"><div className="bf-debt-simulator-head"><div><p className="bf-kicker">SIMULATOR DE DECIZIE</p><h2>Dacă plătești <em>în plus</em>?</h2><span>Testează un efort lunar suplimentar. Nu schimbă datele tale, doar îți arată scenariul.</span></div><div className="bf-debt-simulator-result"><strong>{savedMonths ? "−" + savedMonths : "0"}</strong><small>luni câștigate</small></div></div><div className="bf-debt-slider"><div><span>Sumă extra / lună</span><b>{money(extra)}</b></div><input type="range" min="0" max={maxExtra} step="50" value={extra} onChange={(event) => setExtra(Number(event.target.value))} aria-label="Sumă suplimentară lunară" /><div className="bf-debt-slider-labels"><small>0 RON</small><small>{money(maxExtra)}</small></div></div><div className="bf-debt-simulator-summary"><span><b>{baseMonths}</b><small>luni acum</small></span><span><b>{simulatedMonths}</b><small>luni cu extra</small></span><span><b>{money(minimum + extra)}</b><small>efort lunar</small></span></div></section>;
}
...
export function MoreView({ tab, setTab, data, onChange, onAddReceipt, onDeleteReceipt, onOpenDebt, onOpenSaving, onOpenCalendar, receiptStorageNotice, sync }: { tab: MoreView; setTab: (value: MoreView) => void; data: AppData; onChange: (value: AppData) => void; onAddReceipt: () => void; onDeleteReceipt: (id: string) => void; onOpenDebt: () => void; onOpenSaving: () => void; onOpenCalendar: () => void; receiptStorageNotice?: string; sync: SyncPanelProps }) {
  ...
  const content = () => {
    ...
    if (tab === "review") return <Suspense fallback={<div className="bf-lazy-panel">Pregătim revizuirea…</div>}><ReviewCenterPanel data={data} onChange={onChange} /></Suspense>;
    if (tab === "prices") return <Suspense fallback={<div className="bf-lazy-panel">Pregătim prețurile…</div>}><PriceWatchPanel data={data} onChange={onChange} /></Suspense>;
    if (tab === "pocket") return <Suspense fallback={<div className="bf-lazy-panel">Pregătim buzunarul…</div>}><PocketPanel data={data} /></Suspense>;
    ...
    if (tab === "assistant") return <Suspense fallback={<div className="bf-lazy-panel">Pregătim asistentul…</div>}><AdvisorPanel data={data} onChange={onChange} /></Suspense>;
...
function ExchangeRatesSection({ data, settings, change }: { data: AppData; settings: AppData["settings"]; change: (patch: Partial<AppData["settings"]>) => void }) {
  ...
  return <section><p className="bf-kicker">CURSURI VALUTARE</p><h2>Cât face un leu</h2><p>Se folosesc doar la înregistrarea unei mișcări noi și la soldul inițial al unei surse valutare. Mișcările deja salvate păstrează cursul cu care au fost introduse.</p>
