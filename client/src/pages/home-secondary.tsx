...
export function SettingsPanel({ data, onChange, onReset }: { data: AppData; onChange: (value: AppData) => void; onReset: () => void }) {
  ...
  const exportBackup = (intent: BackupIntent) => downloadBackup(data, intent).then((result) => {
    if (result.how === "shared") window.alert(result.fallback ? t("Telefonul nu a permis scrierea în Documents, așa că am deschis lista de aplicații. Alege Fișiere, Drive sau altă destinație.") : t("Backupul a fost trimis către aplicația aleasă."));
