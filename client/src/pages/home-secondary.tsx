...
function ExchangeRatesSection({ data, settings, change }: { data: AppData; settings: AppData["settings"]; change: (patch: Partial<AppData["settings"]>) => void }) {
  ...
  return <section><p className="bf-kicker">CURSURI VALUTARE</p><h2>{t("Cât face un leu")}</h2><p>{t("Se folosesc doar la înregistrarea unei mișcări noi și la soldul inițial al unei surse valutare. Mișcările deja salvate păstrează cursul cu care au fost introduse.")}</p>
