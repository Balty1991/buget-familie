...
export function TransactionForm({ data, initial, onSave, onClose }: { data: AppData; initial?: Transaction; onSave: (item: Transaction) => void; onClose: () => void }) {
  ...
  const save = () => {
    ...
    if (isForeign && initial && !initial.originalAmount && !(parseRomanianAmount(originalAmountInput) > 0) && numeric > 0) {
      /* keep lei amount as typed; still ask for foreign original below via UI */
    }
