...
export function TransactionForm({ data, initial, onSave, onClose }: { data: AppData; initial?: Transaction; onSave: (item: Transaction | Transaction[], meta?: { fromWeekIndex?: number }) => void; onClose: () => void }) {
  ...
  const envelopeWeeks = matchedEnvelope && matchedEnvelope.weeklyPace !== false ? allocationWeeksStatus(data, matchedEnvelope) : [];
  ...
  const hideUnallocated = kind === "expense" && envelopeCandidates.length > 0 && unrepartized < Math.max(0.005, proposedAmount);
  ...
  useEffect(() => {
    ...
    if (hideUnallocated && allocationId === "outside" && envelopeCandidates[0]) {
      setAllocationId(envelopeCandidates[0].id);
      return;
    }
    ...
    if (canSplit && splitOpen) {
      ...
      const batch = normalized.map((line, index) => {
        const matched = matchingAllocationsForExpense(data, { category: line.category, memberId: member.id, sourceId: source.id })[0]
          || pickerAllocationsForExpense(data, { category: line.category, memberId: member.id, sourceId: source.id })[0];
        return {
...
export function MoreView({ tab, setTab, data, onChange, onAddReceipt, onDeleteReceipt, onOpenDebt, onOpenSaving, onOpenCalendar, onEditDebt, onEditSaving, onPayDebt, onGo, receiptStorageNotice, sync }: { tab: MoreView; setTab: (value: MoreView) => void; data: AppData; onChange: (value: AppData) => void; onAddReceipt: () => void; onDeleteReceipt: (id: string) => void; onOpenDebt: () => void; onOpenSaving: () => void; onOpenCalendar: () => void; onEditDebt?: (item: Debt) => void; onEditSaving?: (item: SavingsGoal) => void; onPayDebt?: (item: Debt) => void; onGo?: (view: MainView) => void; receiptStorageNotice?: string; sync: SyncPanelProps }) {
        ...
        <>
          ...
          <section className="bf-more-group" aria-labelledby="more-simple-title">
            ...
            <div className="bf-more-grid bf-settings-group">
              ...
              <button type="button" className="bf-settings-row" onClick={() => setTab("guide")}><BookOpen size={20} /><span className="bf-settings-copy"><b>{t("Ghid")}</b><small>{t("plic, sursă, reper")}</small></span><ChevronRight className="bf-settings-chevron" size={18} aria-hidden="true" /></button>
            </div>
          </section>
...
export function SettingsPanel({ data, onChange, onReset }: { data: AppData; onChange: (value: AppData) => void; onReset: () => void }) {
  ...
  const addMember = () => { if (!member.trim()) return; if (settings.members.some((item) => item.name.toLowerCase() === member.trim().toLowerCase())) return; if (!canAddMember(settings.members.length)) { window.alert(t("Casa e pentru un membru. Planul Familia acoperă până la {n} persoane.", { n: String(PLANS.familie.members) })); return; } change({ members: [...settings.members, { id: newId("member"), name: member.trim() }] }); setMember(""); };
