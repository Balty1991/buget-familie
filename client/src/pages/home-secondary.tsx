...
function SourceRow({ data, source, settings, change }: { data: AppData; source: AppData["settings"]["paymentSources"][number]; settings: AppData["settings"]; change: (patch: Partial<AppData["settings"]>) => void }) {
  ...
  const remove = () => {
    if (settings.paymentSources.length <= 1) { window.alert("Păstrează cel puțin o sursă de plată."); return; }
    const question = used
  ...
  return <div className="bf-source-edit rich">
    ...
    <div className="bf-source-edit-foot">
      <span><b>{money(sourceBalance(data, source.id))}</b><small>{sourceKindName[source.kind]}{own ? ` · ${own.amount.toLocaleString(getLocale(), { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${own.currency}${own.exact ? "" : ` (${t("aproximativ")})`}` : ""} · {t("{count} mișcări", { count: used })}</small></span>
